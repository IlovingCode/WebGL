...
import { Soldier, checkBound } from 'game/Soldier';
...
class TestScene extends Scene {
  ...
  alphaEffect(dt) {
    if (this.g.t < 1) {
      ...
      this.g.clear();
      if (this.g.t < 1) {
        this.g.beginFill(0x000000, 1 - Math.abs(this.g.t));
        this.g.drawRect(0, 0, display.width, display.height);
        this.g.endFill();
      }
    }
  }
  ...
  update(dt) {
    ...
    checkBound(this.bound, this.character.sprite);

    for (let i = this.enemy.length - 1; i >= 0; i--) {
      const e = this.enemy[i];
      e.update(dt);
      checkBound(this.bound, e.sprite);
    }
    ...
    for (let i = this.bullet.length - 1; i >= 0; i--) {
      ...
      for (let j = this.enemy.length - 1; j >= 0; j--) {
        ...
        const y = e.position.y - b.position.y - e.height * (e.anchor.y - 0.5);
